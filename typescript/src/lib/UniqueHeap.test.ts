...
import { initMinNumberHeap, initMaxNumberHeap, UniqueHeap } from "./UniqueHeap.js"
...
describe("UniqueHeap", () => {
  describe("should pass all of the same tests that the normal Heap does", () => {
    ...
    describe("leetcode integration test (leetcode problem 703)", () => {
      class KthLargest {
        heap: UniqueHeap<TestObj>
        k: number

        constructor(k: number, nums: number[]) {
          this.heap = new UniqueHeap<TestObj>(minTestObjCompare)
          this.k = k
          nums.forEach((val) => {
            this.add(val)
          })
      ...
      // prettier-ignore
      const methods: ("KthLargest" | "add")[] = ["KthLargest","add","add","add","add","add","add","add","add","add","add","add","add","add","add","add","add","add","add","add","add","add","add","add","add","add","add","add"]
      // prettier-ignore
      ...
      beforeAll(() => {
        ...
        methods.forEach((method, i) => {
          ...
          if (method === "KthLargest") {
            instance = new KthLargest(nextArgs[0] as number, nextArgs[1] as number[])
            output.push(null)
          } else {
            output.push(instance[method](nextArgs[0] as number))
          }
        })
      })
  ...
  describe("leetcode integration test. Problem 743, uses Dijkstra's", () => {
    ...
    function networkDelayTime(times: number[][], n: number, k: number): number {
      ...
      for (let nextId: NodeId = 1; nextId <= n; nextId++) {
        graph.set(nextId, {
          id: nextId,
          edges: new Map(),
          distance: Infinity,
      ...
      const minDistancePriorityQueue = new UniqueHeap<Node>(minDistanceCompare)
      ...
      while (minDistancePriorityQueue.size() > 0) {
        ...
        for (const [targetId, weight] of node.edges) {
          ...
          if (newDistance < targetNode.distance) {
            const targetUpdate: Node = { ...targetNode, distance: newDistance }
